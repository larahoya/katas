...
class MarsRover:
    def __init__(self, point: Point, direction: Direction, grid: Grid):
        self.point = point
        self.direction = direction
        self.grid = grid

    def execute(self, command: str) -> (Point, Direction):
        ...
        for i in command:
            match self.direction, i:
                case Direction.north, "M":
                    if self.grid.isobstacle(self.point.x, self.point.y + 1):
                        return self.point, self.direction
                    self.point.y += 1
                    if self.point.y >= self.grid.size:
                ...
                case Direction.east, "M":
                    if self.grid.isobstacle(self.point.x + 1, self.point.y):
                        return self.point, self.direction
                    self.point.x += 1
                    if self.point.x >= self.grid.size:
                        return wrap_point, wrap_direction
